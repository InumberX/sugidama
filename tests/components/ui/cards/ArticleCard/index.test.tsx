...
describe('ArticleCard', () => {
  ...
  describe('Input/Output', () => {
    describe('基本的なカード', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'テスト記事タイトル' }} />
          </MemoryRouter>
        )
      })
    ...
    describe('description がある場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} description={{ text: 'これは説明文です' }} />
          </MemoryRouter>
        )
      })
    ...
    describe('tags が空の場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} tags={[]} />
          </MemoryRouter>
        )
      })

      test('tags 領域は出力されていない', () => {
        const tagsContainer = result.container.querySelector('div[class*="articleCardTags"]')
    ...
    describe('thumbnail がある場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} thumbnail={{ src: '/test-image.jpg', alt: 'テスト画像' }} />
          </MemoryRouter>
        )
      })
    ...
    describe('カスタム titleTag', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル', titleTag: 'h3' }} />
          </MemoryRouter>
        )
      })
    ...
    describe('カスタム articleTag', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} articleTag="section" />
          </MemoryRouter>
        )
      })
  ...
  describe('Display', () => {
    describe('ベーススタイル', () => {
      ...
      test('articleCard クラスが適用されている', () => {
        const article = result.container.querySelector('article')
        expect(article?.className).toContain('articleCard')
      })
    })
    ...
    describe('カスタムクラス名', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} className="custom-class" />
          </MemoryRouter>
        )
      })
    ...
    describe('button に url がある場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} button={{ url: '/path' }} />
          </MemoryRouter>
        )
      })
      ...
      test('clickable クラスが適用されている', () => {
        ...
        expect(titleLink?.className).toContain('articleCard_clickable')
      })
    })
    ...
    describe('button に onClick がある場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} button={{ onClick: () => handleClick() }} />
          </MemoryRouter>
        )
      })
      ...
      test('clickable クラスが適用されている', () => {
        ...
        expect(titleButton?.className).toContain('articleCard_clickable')
      })
    })
    ...
    describe('button がない場合', () => {
      ...
      test('タイトルに clickable クラスは適用されない', () => {
        ...
        expect(titleSpan?.className).not.toContain('articleCard_clickable')
      })
    })
    ...
    describe('内部構造', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} description={{ text: '説明' }} tags={[{ children: 'タグ' }]} />
          </MemoryRouter>
        )
      })

      test('articleCardTitle クラスを持つ要素が存在する', () => {
        const title = result.container.querySelector('h2[class*="articleCardTitle"]')
        expect(title).not.toBe(null)
      })

      test('articleCardDescription クラスを持つ要素が存在する', () => {
        const description = result.container.querySelector('p[class*="articleCardDescription"]')
        expect(description).not.toBe(null)
      })

      test('articleCardTags クラスを持つ要素が存在する', () => {
        const tags = result.container.querySelector('div[class*="articleCardTags"]')
        expect(tags).not.toBe(null)
      })

      test('articleCardThumbnail クラスを持つ要素が存在する', () => {
        const thumbnail = result.container.querySelector('div[class*="articleCardThumbnail"]')
    ...
    describe('tags に outlined バリアントが適用される', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} tags={[{ children: 'タグ' }]} />
          </MemoryRouter>
        )
      })
  ...
  describe('Operation', () => {
    describe('内部リンクカード', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} button={{ url: '/internal/path' }} />
          </MemoryRouter>
        )
      })
      ...
      test('サムネイルにも Link コンポーネントが使用されている', () => {
        const thumbnailLink = result.container.querySelector('div[class*="articleCardThumbnail"] a')
    ...
    describe('外部リンクカード', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} button={{ url: 'https://example.com' }} />
          </MemoryRouter>
        )
      })
    ...
    describe('クリックイベントカード', () => {
      ...
      test('サムネイルのクリックイベントが正常に動作している', () => {
        vi.clearAllMocks()
        const thumbnailButton = result.container.querySelector('div[class*="articleCardThumbnail"] button')
    ...
    describe('description ありのリンクカード', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCard title={{ text: 'タイトル' }} description={{ text: '説明文' }} button={{ url: '/path' }} />
          </MemoryRouter>
        )
      })
