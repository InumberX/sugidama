  ...
  describe('Input/Output', () => {
    ...
    describe('空のリスト', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCardList items={[]} />
          </MemoryRouter>
        )
      })

      test('ラッパー要素は存在するがカードは表示されない', () => {
        const wrapper = result.container.querySelector('div[class*="articleCardList"]')
        const cards = result.container.querySelectorAll('article')
    ...
    describe('単一アイテム', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCardList items={[{ title: { text: '単一記事' } }]} />
          </MemoryRouter>
        )
      })
  ...
  describe('Display', () => {
    describe('ベーススタイル', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCardList items={[{ title: { text: 'テスト' } }]} />
          </MemoryRouter>
        )
      })

      test('articleCardList クラスが適用されている', () => {
        const wrapper = result.container.firstElementChild
        expect(wrapper?.className).toContain('articleCardList')
      })
    })
    ...
    describe('カスタムクラス名', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <ArticleCardList items={[{ title: { text: 'テスト' } }]} className="custom-class" />
          </MemoryRouter>
        )
      })
    ...
    describe('内部構造', () => {
      ...
      test('articleCardList_items クラスを持つ要素が存在する', () => {
        const items = result.container.querySelector('div[class*="articleCardList_items"]')
        expect(items).not.toBe(null)
      })

      test('articleCardList_item クラスを持つ要素が存在する', () => {
        const item = result.container.querySelector('div[class*="articleCardList_item"]')
    ...
    describe('複数アイテムの構造', () => {
      ...
      test('各アイテムが articleCardList_item でラップされている', () => {
        const itemsContainer = result.container.querySelector('div[class*="articleCardList_items"]')
