import { render, type RenderResult, cleanup } from '@testing-library/react'
import { MemoryRouter } from 'react-router'
import { describe, vi, beforeEach, afterEach, test, expect } from 'vitest'

import { HelperText } from '~/components/ui/typographies/HelperText'

describe('HelperText', () => {
  let result: RenderResult

  afterEach(() => {
    cleanup()
    vi.clearAllMocks()
  })

  //============================================================================
  // 1. Input/Output
  //============================================================================
  describe('Input/Output', () => {
    describe('基本的なテキスト', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <HelperText text="テストテキスト" />
          </MemoryRouter>
        )
      })

      test('text が正常に出力されている', () => {
        const text = result.container.querySelector('p')
        expect(text).not.toBe(null)
        expect(text?.textContent).toBe('テストテキスト')
      })

      test('デフォルトで p タグが使用されている', () => {
        const p = result.container.querySelector('p')
        expect(p).not.toBe(null)
      })
    })

    describe('カスタムテキストタグ', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <HelperText text="テストテキスト" textTag="span" />
          </MemoryRouter>
        )
      })

      test('指定したタグ（span）が使用されている', () => {
        const span = result.container.querySelector('span')
        expect(span).not.toBe(null)
        expect(span?.textContent).toBe('テストテキスト')
      })

      test('p タグは使用されていない', () => {
        const p = result.container.querySelector('p')
        expect(p).toBe(null)
      })
    })

    describe('ReactNode を text に渡した場合', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <HelperText text={<span data-testid="custom-text">カスタムテキスト</span>} />
          </MemoryRouter>
        )
      })

      test('ReactNode が正常に出力されている', () => {
        const customText = result.getByTestId('custom-text')
        expect(customText).not.toBe(null)
        expect(customText.textContent).toBe('カスタムテキスト')
      })
    })
  })

  //============================================================================
  // 2. Display
  //============================================================================
  describe('Display', () => {
    describe('ベーススタイル', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <HelperText text="テストテキスト" />
          </MemoryRouter>
        )
      })

      test('helperText クラスが適用されている', () => {
        const helperText = result.container.querySelector('p[class*="helperText"]')
        expect(helperText?.className).toContain('helperText')
      })
    })

    describe('カスタムクラス名', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <HelperText text="テストテキスト" className="custom-class" />
          </MemoryRouter>
        )
      })

      test('カスタムクラス名が正常に付与されている', () => {
        const helperText = result.container.querySelector('p[class*="helperText"]')
        expect(helperText?.classList.contains('custom-class')).toBe(true)
      })
    })
  })

  //============================================================================
  // 3. Operation
  //============================================================================
  describe.skip('Operation', () => {})

  //============================================================================
  // 4. Validation (for forms)
  //============================================================================
  describe.skip('Validation', () => {})

  //============================================================================
  // 5. Others (Optional)
  //============================================================================
  describe.skip('Others (Optional)', () => {})
})
